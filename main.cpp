...
// Простая проверка для русских и английских букв
...
int main()
{
    ...
    if(!isValid(key)) {
        cerr << "Ключ недействителен! Используйте только буквы.\n";
        return 1;
    }
    ...
    do {
        ...
        if(op > 2) {
            ...
            if(isValid(text)) {
                ...
                cout << "Операция отменена: неверный текст. Используйте только буквы.\n";
            }
        }
    } while(op != 0);
